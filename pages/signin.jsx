...
const Signin = () => {
  ...
  return(
     ...
     <div className= 'absolute top-[10%] mx-auto left-0 right-0 w-[450px] p-16 bg-black/80 rounded text-white'>
       ...
       {
       error ? <p className= 'p-3 bg-red-500 my-5'>{error}</p> : null
       }
       <form clasName= 'w-full' onSubmit= {handleSubmit}>
        <input 
        ...
        <input 
          ...
          />
         <br></br>
        <button className= 'bg-red-800 block rounded p-3 w-full mb-8'>SignIn</button>
        <div className= 'w-full flex justify-between text-gray-600'>
          <div>
            <input type="checkbox" id="remember" name="vehicle1" value="Bike" />
            <label className= '' for='remember'> Remember Me</label>
          </div>
          <span className= ''>Need Help?</span>
        </div>
        <p className= 'my-5'>Not subscribed to Netflix? <Link href='/signup'><span className= 'text-blue-500 cursor-pointer hover:underline'>Sign Up</span></Link></p>
